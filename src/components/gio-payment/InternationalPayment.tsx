...
const InternationalPayment: React.FC = () => {
  ...
  const amount = 5; // Fixed amount for international payment in dollars
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { data: order } = await axios.post(
        `${process.env.NEXT_PUBLIC_API_HOSTNAME}/api/gio-event/international-gio-payment/create`,
        { amount },
        { headers: { Authorization: `Bearer ${parsedToken}` } }
      );
      ...
      const options: RazorpayOptions = {
        ...
        name: 'ISRC',
        description: 'International Payment of 5 USD',
        order_id: order.data.id,
        handler: async (response) => {
          try {
            await axios.post(
              `${process.env.NEXT_PUBLIC_API_HOSTNAME}/api/gio-event/international-gio-payment/confirm`,
              {
    ...
    <div className="container d-flex justify-content-center align-items-center min-vh-100">
      <div className="card shadow-lg p-4 mb-5 bg-white rounded" style={{ maxWidth: '500px', width: '100%' }}>
        <div className="text-center mb-4">
          <Image src={logo} alt="Company Logo" width={150} height={50} style={{ display: 'block', margin: '0 auto' }} />
        </div>
        <h5 className="card-title mb-4 text-center" style={{ fontSize: '1.5rem' }}>International Payment</h5>
        ...
        >
          {isLoading ? 'Processing...' : 'Pay 5 Dollar'}
        </button>
      </div>
    </div>
  );
